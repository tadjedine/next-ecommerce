    ...
    <main className="min-h-screen pt-32 pb-24 bg-slate-50">
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div className="mb-8 flex items-center gap-4">
          <div className="w-16 h-16 bg-white rounded-2xl shadow-sm border border-slate-200 flex items-center justify-center text-primary">
            <User size={32} />
          </div>
          <div>
            <h1 className="text-3xl font-extrabold text-navy">My Account</h1>
            <p className="text-slate-500">Manage your addresses and checkout preferences.</p>
          </div>
        </div>
          ...
          <div className="w-full md:w-64 shrink-0">
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-2 space-y-1">
              <button 
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-medium text-sm transition-colors ${activeTab === "addresses" ? "bg-primary text-white shadow-md shadow-primary/20" : "text-slate-600 hover:bg-slate-50"}`}
              >
                <MapPin size={18} /> Addresses
              </button>
              <button 
                onClick={() => setActiveTab("carrier")}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-medium text-sm transition-colors ${activeTab === "carrier" ? "bg-primary text-white shadow-md shadow-primary/20" : "text-slate-600 hover:bg-slate-50"}`}
              >
                <Truck size={18} /> Delivery Carrier
              </button>
              <button 
                onClick={() => setActiveTab("payment")}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-medium text-sm transition-colors ${activeTab === "payment" ? "bg-primary text-white shadow-md shadow-primary/20" : "text-slate-600 hover:bg-slate-50"}`}
              >
              ...
              <AnimatePresence mode="wait">
                {activeTab === "addresses" && (
                  <motion.div key="addresses" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                    <div className="flex items-center justify-between">
                      <h2 className="text-xl font-bold text-navy">My Addresses</h2>
                      <button 
                    ...
                    {loading ? (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {[1, 2].map(i => <div key={i} className="animate-pulse bg-slate-100 h-40 rounded-xl"></div>)}
                      </div>
                    ) : addresses.length === 0 ? (
                      <div className="text-center py-12 px-4 bg-slate-50 rounded-xl border border-dashed border-slate-300">
                        ...
                        <h3 className="text-base font-bold text-navy">No addresses yet</h3>
                        <p className="text-sm text-slate-500 mt-1">Add an address to make your checkout faster.</p>
                      </div>
                    ) : (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {addresses.map(addr => (
                ...
                {activeTab === "payment" && (
                  <motion.div key="payment" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6 max-w-md">
                    <div>
                      <h2 className="text-xl font-bold text-navy mb-1">Preferred Payment Method</h2>
                      <p className="text-sm text-slate-500">This payment method will be pre-selected during checkout.</p>
                    </div>
